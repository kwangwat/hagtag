...
abstract class MainModel {
    ...
    public function fetchAll2Paginator() {
        $tablename = $this->getMapper()->getDbTable()->getTableName();
        return $this->select2Paginator($this->getMapper()->getDbTable()->select()->from($tablename));
    ...
    public function fetch2Options($fields = null) {
        if(! $fields) {
            $fields = $this->getColumnsList();
            $fields = array_keys($fields);
        }
        $select = $this->getSelect()->from($this->getTableName(), array(
            $fields[0], $fields[1]
        ));
        return $this->getMapper()->getDbTable()->getAdapter()->fetchPairs($select);
    }
    ...
    public function getAdapter() {
        if(! $this->_mapper) {
           ...
           return $this->getMapper()->getDbTable()->getAdapter();
        }
    }
