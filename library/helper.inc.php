...
function parseCookie($cookies = array()) {
    $_cookies = array();
    if (is_array($cookies)) {
...
function _print($data) {
    if ($data) {
        if (is_array($data) or is_object($data)) {
            echo "<pre id=\"__DEBUG__\">\n";
            print_r($data);
            echo "</pre>\n";
        } else if (is_bool($data)) {
            var_dump($data);
        } else {
            echo "<div id=\"__DEBUG__\">{$data}</div>\n";
        }
    } else {
        echo "Data is empty\n";
    }
 ...
 * - please do not republish it without due credit and hyperlink to http://detectmobilebrowsers.mobi/ really,
 * i'd prefer it if it wasn't republished in full as that way it's main source is it's homepage and it's always kept up to date
 *
 * For help generating the function call visit http://detectmobilebrowsers.mobi/ and use the function generator. If you need serious help with this
 * please drop me an email to andy@andymoore.info with the subject 'DETECTION CODE PAID SUPPORT REUQEST' with a detailed outline of what you need
 * and how I can help and I will get back to you with a proposal for integration.
 *
 ...
 *
 * To submit a support request please forward your PayPal receipt with your questions to the email address you sent the money to and I will endeavour
 * to get back to you. It might take me a few days but I reply to all support issues with as much helpful info as I can provide.
 * Though really everything is published on the site.
 *
 ...
 *
 * iPhone - Set to true to treat iPhones as mobiles, false to treat them like full browsers or set a URL (including http://)
 * to redirect iPhones and iPods to.
 * Android - Set to true to treat Android handsets as mobiles, false to treat them like full browsers or set a URL (including http://)
 * to redirect Android and Google mobile users to.
 * Opera Mini - Set to true to treat Opera Mini like a mobile, false to treat it like full browser or set a URL (including http://)
 * to redirect Opera Mini users to.
 * Blackberry - Set to true to treat Blackberry like a mobile, false to treat it like full browser or set a URL (including http://)
 * to redirect Blackberry users to.
 * Palm - Set to true to treat Palm OS like a mobile, false to treat it like full browser or set a URL (including http://)
 * to redirect Palm OS users to.
 * Windows - Set to true to treat Windows Mobiles like a mobile, false to treat it like full browser or set a URL (including http://)
 * to redirect Windows Mobile users to.
 * Mobile Redirect URL - This should be full web address (including http://) of the site (or page) you want to send mobile visitors to.
 * Leaving this blank will make the script return true when it detects a mobile.
 * Desktop Redirect URL - This should be full web address (including http://) of the site (or page) you want to send non-mobile visitors to.
 * Leaving this blank will make the script return false when it fails to detect a mobile.
 *
 * bug fixes with many thanks and much credit to http://www.punchkickinteractive.com/ - thanks Ryan!
 *
 * @param boolean $iphone Restricted check for iPhone (default: true)
 * @param boolean $ipad Restricted check for iPad (default: true)
 * @param boolean $android Restricted check for android platform (default: true)
 * @param boolean $opera Restricted check for Opera browser (default: true)
 * @param boolean $blackberry Restricted check for Blackberry platform (default: true)
 * @param boolean $palm Restricted check for Palm platform (default: true)
 * @param boolean $mobilerediract Set for mobile redirection (default: false)
 * @param boolean $desktoprediract Set for desktop redirection (default: false)
 * @return array Collection mobile information detacted
 */
function mobile_device_detect($iphone = true, $ipad = true, $android = true, $opera = true, $blackberry = true, $palm = true, $windows = true,
    ...
    switch (($user_agent || $accept)) {
        ...
        case (preg_match('/ipad/i', $user_agent)): // we find the word ipad in the user agent
            ...
            if (substr($ipad, 0, 4) == 'http') {
                // does the value of ipad resemble a url
                $mobileredirect = $ipad;
                //set the mobile redirect url to the url value stored in the ipad value
            }
            //ends the if for ipad being a url
            break;
        //break out and skip the rest if we've had a match on the ipad
        // this goes before the iphone to catch it else it would return on the iphone instead
        ...
        case (preg_match('/(pre\/|palm os|palm|hiptop|avantgo|plucker|xiino|blazer|elaine)/i', $user_agent)):
            // we find palm os in the user agent - the i at the end makes it case insensitive
        ...
        case (preg_match('/(mini 9.5|vx1000|lge |m800|e860|u940|ux840|compal|wireless| mobi|ahong|lg380|lgku|lgu900|lg210|lg47|lg920|lg840|lg370' .
            '|sam-r|mg50|s55|g83|t66|vx400|mk99|d615|d763|el370|sl900|mp500|samu3|samu4|vx10|xda_|samu5|samu6|samu7|samu9|a615|b832|' .
            'm881|s920|n210|s700|c-810|_h797|mob-x|sk16d|848b|mowser|s580|r800|471x|v120|rim8|c500foma:|160x|x160|480x|x640|t503|w839|' .
            'i250|sprint|w398samr810|m5252|c7100|mt126|x225|s5330|s820|htil-g1|fly v71|s302|-x113|novarra|k610i|-three|8325rc|8352rc|' .
            'sanyo|vx54|c888|nx250|n120|mtk |c5588|s710|t880|c5005|i;458x|p404i|s210|c5100|teleca|s940|c500|s590|foma|samsu|vx8|vx9|' .
            'a1000|_mms|myx|a700|gu1100|bc831|e300|ems100|me701|me702m-three|sd588|s800|8325rc|ac831|mw200|brew |d88|htc\/|htc_touch|' .
            '355x|m50|km100|d736|p-9521|telco|sl74|ktouch|m4u\/|me702|8325rc|kddi|phone|lg |sonyericsson|samsung|240x|x320|vx10|nokia|' .
            'sony cmd|motorola|up.browser|up.link|mmp|symbian|smartphone|midp|wap|vodafone|o2|pocket|kindle|mobile|psp|treo|fennec)/i', $user_agent)):
            // check if any of the values listed create a match on the user agent - these are some of the most common terms used in agents
            // to identify them as being mobile devices - the i at the end makes it case insensitive
        ...
        case (in_array(strtolower(substr($user_agent, 0, 4)), array(
            ...
            'o2 x' => 'o2 x', 'o2-x' => 'o2-x', 'opwv' => 'opwv', 'owg1' => 'owg1', 'opti' => 'opti', 'oran' => 'oran', 'p800' => 'p800',
            'pand' => 'pand', 'pg-1' => 'pg-1', 'pg-2' => 'pg-2', 'pg-3' => 'pg-3', 'pg-6' => 'pg-6', 'pg-8' => 'pg-8', 'pg-c' => 'pg-c',
            'pg13' => 'pg13', 'phil' => 'phil', 'pn-2' => 'pn-2', 'pt-g' => 'pt-g', 'palm' => 'palm', 'pana' => 'pana', 'pire' => 'pire',
            'pock' => 'pock', 'pose' => 'pose', 'psio' => 'psio', 'qa-a' => 'qa-a', 'qc-2' => 'qc-2', 'qc-3' => 'qc-3', 'qc-5' => 'qc-5',
            'qc-7' => 'qc-7', 'qc07' => 'qc07', 'qc12' => 'qc12', 'qc21' => 'qc21', 'qc32' => 'qc32', 'qc60' => 'qc60', 'qci-' => 'qci-',
            'qwap' => 'qwap', 'qtek' => 'qtek', 'r380' => 'r380', 'r600' => 'r600', 'raks' => 'raks', 'rim9' => 'rim9', 'rove' => 'rove',
            's55/' => 's55/', 'sage' => 'sage', 'sams' => 'sams', 'sc01' => 'sc01', 'sch-' => 'sch-', 'scp-' => 'scp-', 'sdk/' => 'sdk/',
            'se47' => 'se47', 'sec-' => 'sec-', 'sec0' => 'sec0', 'sec1' => 'sec1', 'semc' => 'semc', 'sgh-' => 'sgh-', 'shar' => 'shar',
            'sie-' => 'sie-', 'sk-0' => 'sk-0', 'sl45' => 'sl45', 'slid' => 'slid', 'smb3' => 'smb3', 'smt5' => 'smt5', 'sp01' => 'sp01',
            'sph-' => 'sph-', 'spv ' => 'spv ', 'spv-' => 'spv-', 'sy01' => 'sy01', 'samm' => 'samm', 'sany' => 'sany', 'sava' => 'sava',
            'scoo' => 'scoo', 'send' => 'send', 'siem' => 'siem', 'smar' => 'smar', 'smit' => 'smit', 'soft' => 'soft', 'sony' => 'sony',
            't-mo' => 't-mo', 't218' => 't218', 't250' => 't250', 't600' => 't600', 't610' => 't610', 't618' => 't618', 'tcl-' => 'tcl-',
            'tdg-' => 'tdg-', 'telm' => 'telm', 'tim-' => 'tim-', 'ts70' => 'ts70', 'tsm-' => 'tsm-', 'tsm3' => 'tsm3', 'tsm5' => 'tsm5',
            'tx-9' => 'tx-9', 'tagt' => 'tagt', 'talk' => 'talk', 'teli' => 'teli', 'topl' => 'topl', 'hiba' => 'hiba', 'up.b' => 'up.b',
            'upg1' => 'upg1', 'utst' => 'utst', 'v400' => 'v400', 'v750' => 'v750', 'veri' => 'veri', 'vk-v' => 'vk-v', 'vk40' => 'vk40',
            'vk50' => 'vk50', 'vk52' => 'vk52', 'vk53' => 'vk53', 'vm40' => 'vm40', 'vx98' => 'vx98', 'virg' => 'virg', 'vite' => 'vite',
            'voda' => 'voda', 'vulc' => 'vulc', 'w3c ' => 'w3c ', 'w3c-' => 'w3c-', 'wapj' => 'wapj', 'wapp' => 'wapp', 'wapu' => 'wapu',
            'wapm' => 'wapm', 'wig ' => 'wig ', 'wapi' => 'wapi', 'wapr' => 'wapr', 'wapv' => 'wapv', 'wapy' => 'wapy', 'wapa' => 'wapa',
            'waps' => 'waps', 'wapt' => 'wapt', 'winc' => 'winc', 'winw' => 'winw', 'wonu' => 'wonu', 'x700' => 'x700', 'xda2' => 'xda2',
            'xdag' => 'xdag', 'yas-' => 'yas-', 'your' => 'your', 'zte-' => 'zte-', 'zeto' => 'zeto', 'acs-' => 'acs-', 'alav' => 'alav',
            'alca' => 'alca', 'amoi' => 'amoi', 'aste' => 'aste', 'audi' => 'audi', 'avan' => 'avan', 'benq' => 'benq', 'bird' => 'bird',
            'blac' => 'blac', 'blaz' => 'blaz', 'brew' => 'brew', 'brvw' => 'brvw', 'bumb' => 'bumb', 'ccwa' => 'ccwa', 'cell' => 'cell',
            'cldc' => 'cldc', 'cmd-' => 'cmd-', 'dang' => 'dang', 'doco' => 'doco', 'eml2' => 'eml2', 'eric' => 'eric', 'fetc' => 'fetc',
            'hipt' => 'hipt', 'http' => 'http', 'ibro' => 'ibro', 'idea' => 'idea', 'ikom' => 'ikom', 'inno' => 'inno', 'ipaq' => 'ipaq',
            'jbro' => 'jbro', 'jemu' => 'jemu', 'java' => 'java', 'jigs' => 'jigs', 'kddi' => 'kddi', 'keji' => 'keji', 'kyoc' => 'kyoc',
            'kyok' => 'kyok', 'leno' => 'leno', 'lg-c' => 'lg-c', 'lg-d' => 'lg-d', 'lg-g' => 'lg-g', 'lge-' => 'lge-', 'libw' => 'libw',
            'm-cr' => 'm-cr', 'maui' => 'maui', 'maxo' => 'maxo', 'midp' => 'midp', 'mits' => 'mits', 'mmef' => 'mmef', 'mobi' => 'mobi',
            'mot-' => 'mot-', 'moto' => 'moto', 'mwbp' => 'mwbp', 'mywa' => 'mywa', 'nec-' => 'nec-', 'newt' => 'newt', 'nok6' => 'nok6',
            'noki' => 'noki', 'o2im' => 'o2im', 'opwv' => 'opwv', 'palm' => 'palm', 'pana' => 'pana', 'pant' => 'pant', 'pdxg' => 'pdxg',
            'phil' => 'phil', 'play' => 'play', 'pluc' => 'pluc', 'port' => 'port', 'prox' => 'prox', 'qtek' => 'qtek', 'qwap' => 'qwap',
            'rozo' => 'rozo', 'sage' => 'sage', 'sama' => 'sama', 'sams' => 'sams', 'sany' => 'sany', 'sch-' => 'sch-', 'sec-' => 'sec-',
            'send' => 'send', 'seri' => 'seri', 'sgh-' => 'sgh-', 'shar' => 'shar', 'sie-' => 'sie-', 'siem' => 'siem', 'smal' => 'smal',
            'smar' => 'smar', 'sony' => 'sony', 'sph-' => 'sph-', 'symb' => 'symb', 't-mo' => 't-mo', 'teli' => 'teli', 'tim-' => 'tim-',
            'tosh' => 'tosh', 'treo' => 'treo', 'tsm-' => 'tsm-', 'upg1' => 'upg1', 'upsi' => 'upsi', 'vk-v' => 'vk-v', 'voda' => 'voda',
            'vx52' => 'vx52', 'vx53' => 'vx53', 'vx60' => 'vx60', 'vx61' => 'vx61', 'vx70' => 'vx70', 'vx80' => 'vx80', 'vx81' => 'vx81',
            'vx83' => 'vx83', 'vx85' => 'vx85', 'wap-' => 'wap-', 'wapa' => 'wapa', 'wapi' => 'wapi', 'wapp' => 'wapp', 'wapr' => 'wapr',
            ...
            break; // break even though it's the last statement in the switch so there's nothing to break away from but it seems better
        //to include it than exclude it
    ...
    // some of them suck though and will disregard this....
    ...
    // if redirect (either the value of the mobile or desktop redirect depending on the value of $mobile_browser) is true redirect
    // else we return the status of $mobile_browser
...
#####################################################################
# @link http://www.itnewb.com/author/Andrew ITNewb (more by author) #
...
function genToken($len = 32, $md5 = true) {
    ...
    $chars = array(
        'Q', '@', '8', 'y', '%', '^', '5', 'Z', '(', 'G', '_', 'O', '`', 'S', '-',
        'N', '<', 'D', '{', '}', '[', ']', 'h', ';', 'W', '.', '/', '|', ':', '1',
        'E', 'L', '4', '&', '6', '7', '#', '9', 'a', 'A', 'b', 'B', '~', 'C', 'd',
        '>', 'e', '2', 'f', 'P', 'g', ')', '?', 'H', 'i', 'X', 'U', 'J', 'k', 'r',
        'l', '3', 't', 'M', 'n', '=', 'o', '+', 'p', 'F', 'q', '!', 'K', 'R', 's',
